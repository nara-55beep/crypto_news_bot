"""Frozen GMM-state structure applied independently to ES."""
...
def main() -> int:
    ...
    for pullback in (0.10, 0.15):
        for stop in (0.25, 0.50):
            for start in (30, 60):
                for maximum in (1, 2):
                    cfg = G.GConfig(
                        pullback, stop, "long", 13, start, 300, 13, maximum
                    )
                    trades = G.generate(
                        days, bars, signals, cfg, market="es"
                    )
    ...
    print(
        f"ES sessions={len(days)} bars={len(bars)} signals={len(signals)}"
    )
